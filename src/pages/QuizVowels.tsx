      ...
      <div className="space-y-8">
        ...
        <div className="bg-stone-800/60 border border-stone-700/50 rounded-2xl p-8 text-center space-y-5 max-w-md mx-auto">
          ...
          <p className="text-stone-400 text-sm leading-relaxed">
            ...
            Todas as <strong className="text-red-300">21 vogais</strong> — básicas e compostas.
            Acerte <strong className="text-red-300">{TARGET_SCORE}</strong> e ganhe o badge do dia!
          </p>
          {alreadyBadged && (
          ...
          <button onClick={startQuiz} className="w-full bg-red-600 hover:bg-red-500 text-white font-semibold rounded-xl py-3 transition-colors">
            Começar
          </button>
        </div>
      </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-8 py-16 text-center">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={startQuiz} className="bg-red-600 hover:bg-red-500 text-white font-semibold rounded-xl px-6 py-3 transition-colors">
            Jogar de novo
          </button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 max-w-md mx-auto">
      <div className="flex items-center justify-between">
        <Link to="/quiz" className="text-stone-500 hover:text-stone-300 transition-colors text-sm">← Quiz</Link>
        <div className="flex items-center gap-2 bg-red-900/40 border border-red-700/50 rounded-full px-4 py-1 text-sm text-red-300">
          <span>⭐</span><span>{score} / {TARGET_SCORE}</span>
        </div>
      </div>

      <div className="w-full bg-stone-800 rounded-full h-2">
        <div className="bg-red-500 h-2 rounded-full transition-all duration-300" style={{ width: `${(score / TARGET_SCORE) * 100}%` }} />
      </div>

      <div className="bg-stone-800/60 border border-stone-700/50 rounded-2xl p-8 text-center space-y-5">
        <p className="text-stone-400 text-sm uppercase tracking-widest">Qual vogal você ouviu?</p>
        ...
        {phase === 'wrong' && question && (
          <div className="space-y-4 pt-1">
            <div className="bg-red-900/30 border border-red-700/40 rounded-xl p-4 text-left space-y-1">
              ...
              <p className="text-stone-400 text-sm">{question.correct.sound}</p>
              {question.correct.composition && (
                <p className="text-stone-500 text-xs">Composição: {question.correct.composition}</p>
              )}
              {question.correct.examples?.[0] && (
                <p className="text-stone-500 text-xs pt-1">
                  Ex: <span className="text-stone-300">{question.correct.examples[0].word}</span> — {question.correct.examples[0].meaning}
                </p>
              )}
            </div>
            <button onClick={nextQuestion} className="w-full bg-red-600 hover:bg-red-500 text-white font-semibold rounded-xl py-3 transition-colors">
              Próxima vogal →
            </button>
          </div>
        )}
